...
where
    ...
    MK: SharedValueArray + IntoShares<XorReplicated<MK>>,
    BK: SharedValueArray + IntoShares<XorReplicated<BK>>,
...
fn reconstruct_mod_converted_bits<F: Field, B: SharedValueArray>(
    input: [&[Replicated<F>]; 3],
) -> B {
    debug_assert!(
