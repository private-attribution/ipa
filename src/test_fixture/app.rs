...
use crate::helpers::{ByteArrStream, HelperIdentity, Transport, TransportError};
use crate::helpers::query::{QueryConfig, QueryInput};
use crate::protocol::QueryId;
use crate::query::QueryProcessor;
use crate::secret_sharing::{IntoShares};
use crate::test_fixture::network::{InMemoryNetwork, InMemoryTransport, Network, TransportCallbacks};
...
/// [`TestApp`] is capable of running IPA queries end-to-end using [`InMemoryNetwork`]
...
impl TestApp {
    pub fn new() -> Self {
        let processors = [QueryProcessor::default(), QueryProcessor::default(), QueryProcessor::default()]
            .map(Arc::new);
    ...
    pub async fn execute_query<I, A>(&self, input: I, query_config: QueryConfig) -> Result<[Vec<u8>; 3], Error>
        where I: IntoShares<A>,
              A: IntoBuf
    {
        ...
        let running_query = self.query_processors[0].new_query(&transports[0], query_config).await?;

        // Send inputs and poll for completion
        for (i, input) in helpers_input.into_iter().enumerate() {
            self.query_processors[i].receive_inputs(transports[i].clone(), QueryInput {
        ...
        let r = try_join_all(
            self.query_processors.iter().map(|processor| processor.complete(running_query.query_id).map(|r| r.map(|r| r.into_bytes())))
        ).await?;
    ...
    fn callback(query_processor: Arc<QueryProcessor>) -> TransportCallbacks<'static, T> {
        let callbacks = TransportCallbacks {
            receive_query: {
                ...
                Box::new(move |transport: T, receive_query| Box::pin({
                    // I don't know how to convince the compiler that this block owns
                    // processor
                    let processor = processor.clone();
                    async move {
                        let dest = transport.identity();
                        let r = processor.new_query(&transport, receive_query)
                            .await
            ...
            prepare_query: {
                let processor = query_processor.clone();
                Box::new(move |transport: T, prepare_query| Box::pin({
                    // I don't know how to convince the compiler that this block owns
                    // processor
                    let processor = processor.clone();
                    async move {
                        let dest = transport.identity();
                        processor.prepare(&transport, prepare_query)
                            .await
                            .map_err(|e| TransportError::Rejected {
                                dest,
                                inner: Box::new(e),
