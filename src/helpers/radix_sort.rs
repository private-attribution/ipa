...
#[must_use]
pub fn mpc_sort<F: PrimeField>(shared_secrets: &[Vec<F>]) -> Vec<Vec<F>> {
...
fn helpers_gen_mult_inputs<F: PrimeField>(
...
fn helpers_gen_permutations<F: PrimeField>(
    mult_output: &[Vec<F>],
...
// King functions
fn king_do_mult<F: PrimeField>(mult_inputs: &[Vec<F>]) -> Vec<Vec<F>> {
...
fn mult_protocol<F: PrimeField>(mult_inputs: Vec<Vec<F>>) -> Vec<F> {
    ...
    for mult_input in mult_inputs {
        revealed.push(
            mult_input[0] * F::from(3_u64) - mult_input[1] * F::from(3_u64)
                + mult_input[2] * F::from(1_u64),
        );
    }
    revealed
}

fn get_helper_shares_with_h1_0<F: PrimeField>(mult_protocol_output: &[F]) -> Vec<Vec<F>> {
...
// Helper functions
fn helper_get_mult_input<F: PrimeField>(shares: &[F], quadratic_randoms: &[F]) -> Vec<F> {
...
fn helper_get_permutation_share<F: PrimeField>(mult_output: &[F], linear_randoms: &[F]) -> Vec<F> {
...
fn toggle_row_and_columns_of_shares<F: PrimeField>(shares: &[Vec<F>]) -> Vec<Vec<F>> {
...
mod tests {
    ...
    fn convert_int_to_field_2d<F: PrimeField>(shares: &[Vec<u64>]) -> Vec<Vec<F>> {
        shares
            .iter()
    ...
    fn convert_int_to_field_1d<F: PrimeField>(share: &[u64]) -> Vec<F> {
        share
            .iter()
    ...
    fn get_shares_with_randoms(secret: &[u64], randoms: &[u64]) -> Vec<Vec<Fp>> {
    ...
    fn radix_sort() {
        ...
        let expected_sort_output = convert_int_to_field_1d::<Fp>([5, 1, 6, 2, 3, 4].as_ref());
