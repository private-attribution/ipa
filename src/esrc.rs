...
fn encrypt(rng: &mut (impl RngCore + CryptoRng), m: G1Affine, enc_key: EncryptionKey) -> CipherText {
...
fn sign(rng: &mut (impl RngCore + CryptoRng), cm: CipherText, enc_key: EncryptionKey, sign_key: SigningKey) -> Signature {
...
fn adapt(rng: &mut (impl RngCore + CryptoRng), signature: Signature, r_prime: Scalar) -> Signature {
...
fn verify(verify_key: VerifyKey, enc_key: EncryptionKey, cm: CipherText, signature: Signature) -> bool {
    let Signature(z, s, s_hat, t) = signature;
    let CipherText(c0, c1) = cm;
