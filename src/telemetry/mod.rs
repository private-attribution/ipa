...
pub mod metrics {
    ...
    /// Create a tracing span and add current step/role as span fields. These fields will
    /// be propagated for every children span created within this one.
    ///
    ...
    macro_rules! span {
        ($name:expr, step=$step:expr) => {{
            ...
            if let Some(meta) = tracing::Span::current().metadata() {
                match meta.level() {
                    &tracing::Level::DEBUG => tracing::debug_span!($name, step = $step.as_ref()),
                    _ => tracing::info_span!($name, step = $step.as_ref()),
                }
            } else {
                tracing::debug_span!($name, step = $step.as_ref())
            }

            #[cfg(not(test))]
            tracing::debug_span!($name, step = $step.as_ref())
        }};
        ($name:expr, role=$role:expr) => {{
            #[cfg(test)]
            if let Some(meta) = tracing::Span::current().metadata() {
                match meta.level() {
                    &tracing::Level::DEBUG => tracing::debug_span!($name, role = $role.as_ref()),
                    _ => tracing::info_span!($name, role = $role.as_ref()),
                }
            } else {
                tracing::debug_span!($name, role = $role.as_ref())
            }
        ...
        ($name:expr, step=$step:expr, role=$role:expr) => {{
            ...
            if let Some(meta) = tracing::Span::current().metadata() {
                match meta.level() {
                    &tracing::Level::DEBUG => {
                        tracing::debug_span!($name, role = $role.as_ref(), step = $step.as_ref())
                    }
                    _ => tracing::info_span!($name, role = $role.as_ref(), step = $step.as_ref()),
                }
            } else {
                tracing::debug_span!($name, step = $step.as_ref())
            }
