...
use crate::protocol::ipa_prf::malicious_security::prover::LegitProofGenerator;
...
#[derive(Debug)]
pub struct ProofsToVerify<F, const P: usize>
where
    F: PrimeField,
{
    proof_from_left: Vec<[F;P]>,
    proof_from_right: Vec<[F;P]>,
    masks: (F, F),
}

impl<F, const P: usize> ProofsToVerify<F, P>
where
    F: PrimeField,
{
    /// This function generates the `ProofsToVerify`.
    /// The helper party fist generates its own proof and then distributes
    /// shares of the proof to the helper on the left. The shares for the helper on the right
    /// are generated using `PRSS`. Further, this helper
    /// receives a proof from the right helper, i.e. `proof_from_right`,
    /// and generates a proof using `PRSS`, i.e. `proof_from_left`.
        ...
        I: Iterator<Item = UVTupleBlock<F>> + Clone,
    {
        ...
        let (prover_left_batch, verifier_left_batch, (p, q)) =
            Proof::<F, R>::compute_proof_batch(&ctx, uv_tuple_block);
        ...
        // output
        ProofsToVerify {
            proof_from_left: verifier_left_batch.components,
            proof_from_right: verifier_right_batch.components,
            masks: (p, q),
        }
    }
}

/// This a `Proof` generated by a prover.
struct Proof<F, const P: usize>
where
    F: PrimeField,
{
    components: Vec<[F; P]>,
...
impl<F, const P: usize> Proof<F, P>
where
    F: PrimeField + Default,
{
    fn len(&self) -> usize {
        self.components.len() * P
    }

    fn new() -> Self {
        Proof { components: Vec::new() }
    }
        ...
        C: Context,
    {
        ...
        // let mut receive_right = Proof::new();

        // send to left, receive on the right
        // we send the proof batch via sending the individual field elements
        let components = seq_join(
            ctx_left.active_work(),
            stream::iter(self.components.iter().flat_map(|x| x.iter()).enumerate().map(
                |(i, &x)| async move {
        ...
        .collect::<Vec<[F; P]>>().await;
        // .collect::<Vec<F>>()
        // .await
        // .chunks(<ZeroKnowledgeProof<F, R> as ProofArray>::Length::USIZE)
        // .for_each(|x| {
        //     receive_right.components.push(ZeroKnowledgeProof {
    ...
    {
        let mut prover_left_batch = Vec::<[F; P]>::new();
        let mut verifier_left_batch = Vec::<[F; P]>::new();
        ...
        (
            Proof {
                components: prover_left_batch,
            },
            Proof {
                components: verifier_left_batch,
            },
    ...
    fn compute_next_proof<C, J, B, const λ: usize, const N: usize, const M: usize>(
        prover_left_batch: &mut Vec<[F; P]>,
        verifier_left_batch: &mut Vec<[F; P]>,
        record_counter: &mut RecordId,
        ctx: &C,
        lagrange_table: &LagrangeTable<F, N, M>,
        uv: J,
    ) -> Vec<([F; λ],[F; λ])>
    where
        C: Context,
        ...
        B: Borrow<UVPolynomial<F, R>>,
    {
        // generate next proof
        // from iterator
        let mut prover_left_proof = LegitProofGenerator::compute_proof(uv.clone(), lagrange_table);
        ...
        C: Context,
    {
        ...
        let mut prover_right_proof = [F::ZERO; P];
        let mut verifier_left_proof = [F::ZERO; P];
...
mod test {
    ...
    use crate::{
        ...
        protocol::{
            ...
            ipa_prf::{
                ...
                validation_protocol::proof_generation::ProofsToVerify,
            },
        },
        ...
        run(|| async move {
            ...
            let result = world
                .semi_honest(h, |ctx, h| async move {
                    ...
                    (
                        h,
                        ProofsToVerify::<Fp61BitPrime, U8>::generate_and_distribute_proofs(
                            ctx,
    ...
    fn simple_proof_check<R>(
        h: Fp61BitPrime,
        left_verifier: &ProofsToVerify<Fp61BitPrime, R>,
        right_verifier: &ProofsToVerify<Fp61BitPrime, R>,
    ) where
        R: ArrayLength,
    {
        // check lengths
        // i.e. each proof has length 2*R::SIZE -1
        for i in 0..left_verifier.proof_from_right.len() {
            assert_eq!((i, left_verifier.proof_from_right[i].g.len()), (i, 2 * R::USIZE - 1));
        }
]
        // check that masks are not 0
        assert_ne!(
            right_verifier.masks,
        ...
        let proof_computed = left_verifier.proof_from_right[0]
            .g
            .iter()
            .zip(right_verifier.proof_from_left[0].g.iter())
