...
impl<F: PrimeField, const L: usize, const P: usize, const M: usize> ProofGenerator<F, L, P, M> {
    ...
    fn gen_proof_shares_from_prss<C>(
        ctx: &C,
        record_counter: &mut RecordIdRange,
    ) -> ([F; P], [F; P])
    where
        C: Context,
    {
        ...
        for i in 0..P {
            let (left, right) = ctx
                .prss()
                .generate_fields::<F, RecordId>(record_counter.next().unwrap());
...
mod test {
    ...
    fn check_uv_length() {
        run(|| async move {
            ...
            let mut record_counter = RecordIdRange::ALL;
            let (uv_values, _, _) =
    ...
    pub async fn test_prss_consistency() {
        ...
        let [helper_1_proofs, helper_2_proofs, helper_3_proofs] = world
            .semi_honest((), |ctx, ()| async move {
                ...
                (0..NUM_PROOFS)
                    .map(|i| {
                        assert_eq!(i * 7, usize::from(record_counter.peek_first()));
