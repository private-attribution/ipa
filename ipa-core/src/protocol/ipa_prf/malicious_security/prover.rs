...
impl<F: PrimeField, const λ: usize, const P: usize, const M: usize> ProofGenerator<F, λ, P, M> {
        ...
        B: Borrow<([F; λ], [F; λ])>,
    {
        // generate next proof
        // from iterator
        let prover_proof = Self::compute_proof(uv.clone(), lagrange_table);

        // generate proof shares from prss
        let (proof_from_left, prover_right_proof) =
            Self::gen_proof_shares_from_prss(ctx, record_counter);

        // generate prover left proof
        let prover_left_proof = Self::gen_other_proof_share(prover_proof, prover_right_proof);
