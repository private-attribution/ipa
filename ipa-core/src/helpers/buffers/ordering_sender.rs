...
mod test {
    ...
    fn send_close_recv() {
        run(|| async {
            ...
            let output = Fp31::deserialize_infallible(GenericArray::from_slice(&flat));
    ...
    fn mixed_size() {
        run(|| async {
            ...
            let (_, taken) = join(close_send, sender.as_stream().collect::<Vec<_>>()).await;
            let flat = taken.into_iter().flatten().collect::<Vec<_>>();
            let small_out = Fp31::deserialize_infallible(GenericArray::from_slice(&flat[..1]));
            assert_eq!(small_out, small);
            let large_out =
                Fp32BitPrime::deserialize_infallible(GenericArray::from_slice(&flat[1..]));
    ...
    fn random_fp32bit() {
        ...
        run(|| async {
            ...
            for (&v, b) in zip(values.iter(), buf.chunks(SZ)) {
                assert_eq!(
                    v,
                    Fp32BitPrime::deserialize_infallible(GenericArray::from_slice(b))
                );
            }
        });
    }
    ...
    fn shuffle_fp31() {
        ...
        run(|| async {
            ...
            let buf = output.into_iter().flatten().collect::<Vec<_>>();
            for (&v, b) in zip(values.iter(), buf.chunks(SZ)) {
                assert_eq!(v, Fp31::deserialize_infallible(GenericArray::from_slice(b)));
            }
        });
    }
